...
class JumpBackwardSelected : DumbAwareAction() {
	...
	override fun actionPerformed(event: AnActionEvent) {
		...
		try {
			...
			val targetOffset = getTargetOffsetBackward(document, foldingModel, currentOffset, currentBackwardNoOfLines)

			caretModel.moveToOffset(targetOffset)

			// Capture the original column before moving
			// val originalColumn = caretModel.logicalPosition.column
